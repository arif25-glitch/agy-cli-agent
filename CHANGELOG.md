...
### Added
- **Jev AI Startup Key Validation & Guardrail (`gemini_hermes/jev/client.py`, `run.sh`)**:
  * Added mandatory API key guardrails for Jev AI mode (`TYPESAFE_API_KEY` / `JEV_API_KEY`) with clear user configuration guidance and fallback prompt to pure execution mode.
  * Added comprehensive unit test coverage (`tests/test_jev_startup_validation.py`).
